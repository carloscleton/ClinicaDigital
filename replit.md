...
- June 23, 2025. Fixed dialog opening/closing issue in specialties registration. Corrected onOpenChange handler to properly manage dialog state, resolving immediate close problem that prevented form submission. Removed "Valor" column from patients management table per user request, streamlining interface to show only essential patient information (Nome, Contato, CPF, Status, Data Cadastro, Ações).
