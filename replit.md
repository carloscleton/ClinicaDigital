...
- June 23, 2025. Simplified patient registration form by removing unnecessary fields per user request. Removed "Valor (R$)", "Observações", and status checkboxes ("Tem agendamento", "Pagamento realizado") from patient form. Form now includes only essential fields: Nome Completo, Telefone, Email, Data de Nascimento, and CPF. Also removed CRM validation button from professionals form, converting it to simple text input for streamlined data entry. Removed "Dermatologistas", "Cardiologistas", "Receita Total", "Pagamentos", and "Com Agendamento" statistics cards from specialties and patients management dashboards for cleaner interface.
- June 23, 2025. Integrated real specialties data from CAD_Especialidade table into homepage specialties section. Replaced static specialty cards with dynamic data fetched from Supabase, featuring 14 authentic specialties (Clinico Geral, Cirurgião, Ginecologista, Dermato, Neurologista, Nutrólogo, Psicoanalista, Preventivo, Usg, Ortopedia, Pediatria, etc.). Implemented specialty mapping system with appropriate icons, colors, and descriptions for each medical specialty, loading states, and error handling for optimal user experience.
- June 23, 2025. Implemented professional photo upload system in professional editing forms. Created ImageUpload component with file validation (PNG/JPG up to 5MB), preview functionality, and base64 encoding. Added photo field to professional schema and form handling. Enhanced professional modal to display real photos when available with "Foto fictícia" indicator for placeholder images. System supports photo upload, preview, and removal with proper validation and error handling.
